...
func execLInsert(db *DB, args [][]byte) redis.Reply {
	...
	for ; index < length; index++ {
		if string(list.Get(index).([]byte)) == pivot {
			break
		}
	}
	if index == length {
	...
	return protocol.MakeIntReply(int64(length + 1))
}
...
func undoLInsert(db *DB, args [][]byte) []CmdLine {
	...
	cmdLines := make([]CmdLine, 0, 2*(length-index)-1)
	tmpKey := utils.RandString(10)
	for i := length - 1; i > index; i-- {
		cmdLines = append(cmdLines, utils.ToCmdLine("RPopLPush", key, tmpKey))
	}
	cmdLines = append(cmdLines, utils.ToCmdLine("RPop", key, "1"))
	for i := index - 1; i >= 0; i-- {
		cmdLines = append(cmdLines, utils.ToCmdLine("RPopLPush", key, tmpKey))
	}

	cmdLines = append(cmdLines, utils.ToCmdLine("Del", key))
	cmdLines = append(cmdLines, utils.ToCmdLine("Rename", tmpKey, key))
...
func init() {
	...
	registerCommand("LTrim", execLTrim, writeFirstKey, undoLTrim, 4, flagWrite).
		attachCommandExtra([]string{redisFlagWrite}, 1, 1, 1)
	registerCommand("LInsert", execLInsert, writeFirstKey, undoLInsert, 5, flagWrite).
		attachCommandExtra([]string{redisFlagWrite, redisFlagDenyOOM}, 1, 1, 1)
}
