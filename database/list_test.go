...
func TestLInsert(t *testing.T) {
	...
	expected3 := protocol.MakeErrReply("ERR syntax error")
	if !utils.BytesEquals(result3.ToBytes(), expected3.ToBytes()) {
		t.Errorf("expected %s, actually %s", string(expected3.ToBytes()), string(result3.ToBytes()))
	}
	...
	expected4 := protocol.MakeErrReply("ERR wrong number of arguments for 'linsert' command")
	if !utils.BytesEquals(result4.ToBytes(), expected4.ToBytes()) {
		t.Errorf("expected %s, actually %s", string(expected4.ToBytes()), string(result4.ToBytes()))
	}
	...
	expected5 := protocol.MakeIntReply(-1)
	if !utils.BytesEquals(result5.ToBytes(), expected5.ToBytes()) {
		t.Errorf("expected %s, actually %s", string(expected5.ToBytes()), string(result5.ToBytes()))
	}
}
...
func TestUndoLInsert(t *testing.T) {
	...
	// case1
	cmdLine1 := utils.ToCmdLine("linsert", key, "before", "3")
	undoCmdLines1 := undoLInsert(testDB, cmdLine1[1:])
	testDB.Exec(nil, cmdLine1)
	...
	results1 := testDB.Exec(nil, utils.ToCmdLine("lrange", key, "0", "-1"))
	values1 := []string{"a", "b", "c", "d", "e", "f"}
	asserts.AssertMultiBulkReply(t, results1, values1)

	// case2
	cmdLine2 := utils.ToCmdLine("linsert", key, "after", "3")
	undoCmdLines2 := undoLInsert(testDB, cmdLine2[1:])
	testDB.Exec(nil, cmdLine2)
	...
	results2 := testDB.Exec(nil, utils.ToCmdLine("lrange", key, "0", "-1"))
	values2 := []string{"a", "b", "c", "d", "e", "f"}
	asserts.AssertMultiBulkReply(t, results2, values2)
}
